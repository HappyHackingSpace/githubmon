...
interface ActionItemsState {
  ...
  setLoading: (
    type: keyof ActionItemsState["loading"],
    loading: boolean
  ) => void;
  setError: (
    type: keyof ActionItemsState["errors"],
    error: string | null
  ) => void;
  ...
  persist(
    (set, get) => ({
      ...
      getCountByType: (type) => {
        ...
        switch (type) {
          case "assigned":
            return state.assignedItems.length;
          case "mentions":
            return state.mentionItems.length;
          case "stale":
            return state.staleItems.length;
          case "goodFirstIssues":
            return state.goodFirstIssues.length;
          case "easyFixes":
            return state.easyFixes.length;
          default:
            return 0;
        }
      },
      ...
      refreshData: async (type) => {
        ...
        if (!userToken) {
          ...
          const types = type
            ? [type]
            : (["assigned", "mentions", "stale"] as const);
          for (const t of types) {
            set((state) => ({
              errors: { ...state.errors, [t]: "GitHub token required" },
            }));
          }
          return;
        }
        ...
        if (!username) {
          ...
          for (const t of types) {
            set((state) => ({
              errors: { ...state.errors, [t]: "GitHub username required" },
            }));
          }
          return;
        }
        ...
        try {
          const actionData = await githubGraphQLClient.getActionRequiredItems(
            username
          );
          ...
          if (type) {
            switch (type) {
              case "assigned":
                get().setAssignedItems(
                  actionData.assigned.map((item) => ({
                    ...item,
